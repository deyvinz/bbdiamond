...
const INVITATION_COLUMNS = [
  { id: 'guest_name', label: 'Guest Name', default: true },
  ...
  { id: 'token', label: 'Invitation Token', default: false },
...
export default function ExportDialog({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    INVITATION_COLUMNS.filter(col => col.default).map(col => col.id)
  )
  ...
  const handleSelectAllColumns = () => {
    if (selectedColumns.length === INVITATION_COLUMNS.length) {
      // Deselect all
      setSelectedColumns([])
    } else {
      // Select all
      setSelectedColumns(INVITATION_COLUMNS.map(col => col.id))
    }
  }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/invitations/export?${params.toString()}`)
      ...
      toast({
        ...
        description: 'Your invitations have been exported to CSV',
      })
      ...
      toast({
        title: 'Export failed',
        description: error instanceof Error ? error.message : 'Failed to export invitations',
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
  ...
  const handleReset = () => {
    setEventFilter('all')
    setStatusFilter('all')
    setSelectedColumns(INVITATION_COLUMNS.filter(col => col.default).map(col => col.id))
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Select filters and columns to export invitation data to CSV
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              >
                {selectedColumns.length === INVITATION_COLUMNS.length ? 'Deselect All' : 'Select All'}
              </Button>
            </div>
